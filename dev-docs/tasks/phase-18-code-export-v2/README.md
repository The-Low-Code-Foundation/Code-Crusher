...
| [EXP-014](./EXP-014-THE-GROUND-THE-HEADLINE-SITS-ON.md) | **The ground the headline sits on** | 🟢 **BUILT, GATED, DRIVEN — s97, 2026-09-06.** The two layer ports fold into one `background-image`, gradient first, with the runtime's size/position defaults and `no-repeat`; `backdropBlur` maps to both spellings or neither. Hero headline **1.03:1 → 17.24:1**; 9 refusals gone, none added. **§14.5 CLOSED s99, 2026-09-12**: every media URL — `Image.src`/`srcSet`, `Video.src`/`poster`, a picture icon — is now printed as the viewer's own port rule prints it (`getAbsoluteUrl` behind `resolveMediaSource`, transcribed into `mediaLib.ts`): a literal root-absolute at emit time, an emptied field as no attribute, a WIRE through `mediaSrc()` from `src/lib/media.ts` at run time — the wired path is ~18 of the 20 pictures on the landing-pages template. First corpus fixture two segments deep (`picture-desk`); 26 rows, 15/15 arms. 🔴 One runtime-side residual in its place: the viewer's own `srcset` is verbatim (§14.6, owner NONE) |
