...
| 2026-09-12 | **Rocket School** (TPL-007, a kids' maths + typing game) | the MCP (plan door, via `tpl007Template.ts`) | 42 | 95% | 38% | 0.19 | **25** named utilities in `Logic/`, each Component Inputs → one Function → Component Outputs; 4 `Data/` sources | ✅ PASS ×3 | 🔴 **Five product defects, all invisible with 0 console errors**: a kit React node as a component ROOT draws nothing (D53); an Expression shadows `String`/`Math` with input ports (D54); an Expression with no delivered input never evaluates, so a guarded optional `mounted` hides the part (D55); a Variable is global by name, so two instances of one component share it (D57). The six questions, asked: what repeated was the pill (`Game/Choice`), what was named-but-inlined was the round (`Race/Round`), the utility in disguise was the number speller (kept inside the picker — one caller), the hard-coded-that-wanted-a-port was every colour (all tokens), and what the parts refused to tell their parents was answered by 95% publishing. Pages are 22–30 nodes; the new-player form is 32 and wants a face-picker section |
