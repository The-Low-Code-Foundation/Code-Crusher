...
describe('TPL-008 — the todo list, driven', () => {
  ...
  const until = async <T>(label: string, read: () => Promise<T>, ok: (v: T) => boolean, ms = 20_000): Promise<T> => {
  ...
  const text = async (page: RenderedPage): Promise<string> => String(await page.evaluate('document.body.innerText'));
  const pathname = async (page: RenderedPage): Promise<string> => String(await page.evaluate('location.pathname'));

  /**
   * The Nth button in the smallest element around a piece of text that holds
   * exactly `count` buttons — a task row holds three (up, down, close), a next
   * action's line holds three (tick, up, down). Scrolled into view, then clicked
   * as a real pointer press.
   */
  const clickButtonBeside = async (page: RenderedPage, label: string, count: number, index: number): Promise<void> => {
    const found = String(
  ...
  const clickWords = async (page: RenderedPage, words: string): Promise<void> => {
    const found = String(
      await page.evaluate(`(function () {
  ...
  const clickButtonByField = async (page: RenderedPage, buttonLabel: string, placeholder: string): Promise<void> => {
    const found = String(
      await page.evaluate(`(function () {
  ...
  const blur = async (page: RenderedPage): Promise<void> => {
    await page.evaluate('(function () { if (document.activeElement) document.activeElement.blur(); return true; })()');
    await wait(800);
  };
  const buttonDisabled = async (page: RenderedPage, label: string): Promise<unknown> =>
  ...
  it('§5 a note, a rename and a deadline each leave a line; a deadline that is not a date is refused in words', () => {
    ...
    expect(R.historyKinds).toEqual(
      ['action-added', 'action-described', 'action-done', 'created', 'deadline', 'moved', 'note', 'renamed'].sort()
    );
